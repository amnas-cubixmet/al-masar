...
export default function SelectedBranchCard({ branch }: { branch: Branch }) {
  ...
  return (
    <div className="mt-4 grid overflow-hidden rounded-xl border border-white/10 bg-[#151E2D] sm:mt-8 sm:rounded-2xl lg:grid-cols-[0.9fr_1.1fr]">
      <div className="flex flex-col justify-between p-3.5 sm:p-6 lg:p-8">
        <div>
          ...
          <h3 className="mt-2.5 text-[16px] font-semibold sm:mt-4 sm:text-2xl leading-snug text-white">{branch.name}</h3>
          <p className="mt-1.5 text-[12px] leading-relaxed text-slate-400 sm:mt-2 sm:text-sm">{branch.address}</p>

          <div className="mt-3 flex flex-col gap-1 text-[11px] sm:text-xs text-slate-300 sm:mt-4">
