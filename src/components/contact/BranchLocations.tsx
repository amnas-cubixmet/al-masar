...
export default function BranchLocations({ branches }: BranchLocationsProps) {
  ...
  return (
    <div className="mt-10 pt-8 sm:mt-16 sm:pt-12" dir={isArabic ? "rtl" : "ltr"}>
      <div className="branch-section-title mb-5 sm:mb-8">
        ...
        <p className="mt-1.5 max-w-2xl text-[12px] leading-relaxed text-[#AAB4C3] sm:mt-2 sm:text-sm">
          {isArabic
            ? "تفضل بزيارة فروعنا في الرياض والقصيم وجدة لتلبية احتياجات مشروعك."
            : "Visit or contact our branches in Riyadh, Qassim and Jeddah for your project needs."}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3">
        {branches.map((branch) => {
          ...
          const city = isArabic && branch.cityAr ? branch.cityAr : branch.city;
            ...
            >
              <div>
                <div className="flex items-center justify-between gap-3">
                  <span className="rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-[11px] font-bold text-[#6EA8FF]">
                    {label}
                  </span>
                  <span className="text-xs font-semibold text-[#AAB4C3]">{city}</span>
                </div>

                <h3 className="mt-2.5 text-[16px] font-bold text-white transition group-hover:text-[#6EA8FF] sm:mt-3 sm:text-lg">
                  {name}
                </h3>

                <div className="mt-3 flex items-start gap-2.5 text-xs text-[#AAB4C3] sm:mt-4">
                  <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#6EA8FF]" />
                  <p className="leading-5">{address}</p>
                </div>
                ...
                <div className="mt-3 sm:mt-4">
                  ...
                  <div className="mt-1 flex flex-col gap-1" dir="ltr">
                    {branch.phones.map((phone) => (
                      <a
                        key={phone}
                        href={`tel:${normalizePhone(phone)}`}
                        className="inline-flex items-center gap-1.5 text-xs font-bold text-white transition hover:text-[#6EA8FF]"
                      >
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2 pt-3 sm:mt-6 sm:border-t sm:border-white/5 sm:pt-4">
                <a
                  ...
                  className="inline-flex h-8 items-center gap-1.5 rounded-xl bg-gradient-to-r from-[#6EA8FF] via-[#8A63E8] to-[#C45BCB] px-3 text-[10px] font-bold text-white transition hover:brightness-110 sm:h-9 sm:px-3.5 sm:text-xs"
                >
                ...
                {branch.whatsapp ? (
                  <a
                    href={whatsappHref(
                      branch.whatsapp,
                      isArabic
                        ? `مرحباً ${name}، لدي استفسار بخصوص مشروع.`
                        : `Hello AL MASAR ${branch.name}, I have an enquiry.`
                    )}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex h-8 items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 text-[10px] font-bold text-white transition hover:bg-white/10 sm:h-9 sm:px-3.5 sm:text-xs"
                  >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex h-8 items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 text-[10px] font-bold text-[#AAB4C3] transition hover:border-[#6EA8FF]/40 hover:text-white sm:h-9 sm:px-3.5 sm:text-xs"
                >
